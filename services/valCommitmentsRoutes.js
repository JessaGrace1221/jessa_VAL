...
function registerValCommitmentsRoutes(app,deps={}){
  ...
  const afterCommitmentEvent=typeof deps.afterCommitmentEvent==='function'?deps.afterCommitmentEvent:async()=>{};

  async function notifyCommitmentEvent(event){
    try{
      await afterCommitmentEvent(event);
    }catch(error){
      deps.logger?.warn?.('[val-commitments] Board packet event failed:',error.message);
    }
  }
  ...
  app.post('/api/val/commitments/:id/status',async(req,res)=>{
    try{
      ...
      await notifyCommitmentEvent({eventType:'status_updated',commitmentId:req.params.id,result,commitment:result.commitment,request:req.body||{}});
  ...
  app.post('/api/val/commitments/:id/draft-email',async(req,res)=>{
    try{
      ...
      await notifyCommitmentEvent({eventType:'draft_created',commitmentId:req.params.id,result,commitment:result.commitment,draft:result.draft,request:req.body||{}});
  ...
  app.post('/api/val/commitments/:id/create-task',async(req,res)=>{
    try{
      ...
      await notifyCommitmentEvent({eventType:'task_created',commitmentId:req.params.id,result,commitment:result.commitment,task:result.task,request:req.body||{}});
