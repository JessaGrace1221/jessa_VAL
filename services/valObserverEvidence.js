...
function normalizeObserverEvidenceReview(run={},review={}){
  ...
  return {
    ...
    observerName,
    observerId:String(review.observerId||review.observer_id||run.outputJson?.observerDefinition?.observerId||run.output_json?.observerDefinition?.observerId||''),
    observerVersion:String(review.observerVersion||review.observer_version||run.outputJson?.observerDefinition?.observerVersion||run.output_json?.observerDefinition?.observerVersion||''),
    promptKey:String(run.promptKey||run.prompt_key||run.outputJson?.observerDefinition?.promptKey||run.output_json?.observerDefinition?.promptKey||''),
