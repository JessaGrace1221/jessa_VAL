...
const STOP_NAME_WORDS = new Set([
  ...
  'Document','Documents','Attachment','Attachments','Invoice','Invoices','Receipt','Receipts','Payment','Payments',
  'Google','Workspace','Google Workspace','Google Payments','Your','Available','Account','Accounts',
...
function packetPeople(packet={}){
  ...
  const sourceKey=registrySourceKey(packet.sourceType);
  const capitalized=sourceKey==='document'
    ? []
    : Array.from(text.matchAll(/\b([A-Z][a-z]{2,}(?:\s+[A-Z][a-z]{2,})?)\b/g)).map(match=>match[1]);
...
function observerMeaningfulSignal(observerName='',packet={}){
  ...
  if(observerName==='Relationship'){
    const meaningfulRelationshipTerms=matched.filter(term=>!['relationship','person','people','contact'].includes(term));
    return {meaningful:Boolean(primary||meaningfulRelationshipTerms.length||packetPeople(packet).length),matched:meaningfulRelationshipTerms,primary};
  }
...
function observerReviewLooksPolluted(review={}){
  return safeArray(review.people).some(person=>{
    const value=String(person||'').trim();
    return STOP_NAME_WORDS.has(value) || /^(Document|Documents|Attachment|Invoice|Receipt|Payment|Google|Workspace|Your|Available|Account)$/i.test(value) || /Google (Workspace|Payments)/i.test(value);
  });
}

function observerReviewsNeedRefresh(existing=[]){
  const reviews=safeArray(existing);
  if(reviews.length!==BOARD_OBSERVERS.length)return true;
  return reviews.some(observerReviewLooksPolluted);
}

function withObserverReviews(packet={}){
  const existing=safeArray(packet.payloadJson?.observerReviews);
  if(!observerReviewsNeedRefresh(existing))return packet;
