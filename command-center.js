...
function cardChatPanel(type,item,activeId,chips){
  ...
  return '<section class="exec-card val-card-chat-panel"><h3>Command VAL</h3><div id="valCardChatLog"><div class="val-card-chat">'+safe(spec.intro)+' What would you like to do next?</div></div><div class="val-card-chip-row">'+chips.map(function(ch){return '<button onclick="homepageCardAsk(\''+jsString(type)+'\',\''+jsString(activeId)+'\',\''+jsString(ch.prompt)+'\')">'+safe(ch.label)+'</button>';}).join('')+'</div><div class="val-card-chat-input"><textarea id="valCardChatInput" rows="2" placeholder="Tell VAL what to do from here..." onkeydown="if(event.key===\'Enter\'&&!event.shiftKey){event.preventDefault();homepageCardAsk(\''+jsString(type)+'\',\''+jsString(activeId)+'\')}" oninput="this.style.height=\'auto\';this.style.height=Math.min(this.scrollHeight,118)+\'px\'"></textarea><button onclick="homepageCardAsk(\''+jsString(type)+'\',\''+jsString(activeId)+'\')" aria-label="Send">Send</button></div></section>';
}
