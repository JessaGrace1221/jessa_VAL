...
window.renderTranscriptList=function(){
  ...
  view.innerHTML=transcriptHeader('Notes and transcripts live together here. Select a transcript, review the Notes tab, and keep raw Transcript text as evidence.')+'<div class="val-transcript-workspace list-only">'+transcriptSidebarHtml('')+'<main class="val-transcript-main"><div class="val-transcript-stats"><span class="val-transcript-stat"><strong>'+Number(c.total||transcriptState.items.length)+'</strong> transcripts</span><span class="val-transcript-stat"><strong>'+Number(c.failedProcessing||0)+'</strong> processing issues</span></div><div class="val-transcript-list">'+(rows||'<div class="val-empty"><strong>No real transcripts are available yet.</strong><br>No transcripts are available yet because VAL has not received a usable meeting, voice, VAL conversation, upload, or webhook transcript for this dashboard. Planning notes, prompts, drafts, and task artifacts are intentionally hidden here.</div>')+'</div></main></div>';
};
...
function renderTranscriptDetail(t){
  ...
  view.innerHTML=transcriptHeader(meta,true)+'<div class="val-transcript-workspace">'+transcriptSidebarHtml(t.id)+'<main class="val-transcript-main"><div class="val-transcript-selected-head"><div><h2>'+safe(transcriptShortText(t.title,'Transcript',120))+'</h2><p>'+safe(meta||'Transcript intelligence')+'</p></div><button class="val-ui-btn primary" onclick="transcriptAskFocus()">Chat About This Transcript</button></div><div class="val-transcript-tabs"><button class="active" data-transcript-tab="notes" onclick="setTranscriptTab(\'notes\')">Notes</button><button data-transcript-tab="transcript" onclick="setTranscriptTab(\'transcript\')">Transcript</button></div><section class="val-transcript-panel active" data-transcript-panel="notes">'+transcriptNotesHtml(t,s,tasks,createdTasks,debug)+'</section><section class="val-transcript-panel" data-transcript-panel="transcript"><section class="val-detail-card"><h3>Transcript</h3><p class="val-full-transcript">'+safe(t.transcriptText||t.rawTranscript||'No transcript text is available.')+'</p></section></section></main><aside class="val-detail-side"><section class="val-detail-card"><h3>Chat About This Transcript</h3><div class="val-chat-log" id="valTranscriptChat"><div class="val-chat-msg">Ask about what happened, what was decided, what matters, or what VAL noticed in this transcript.</div></div><div class="val-chat-input"><input id="valTranscriptQuestion" placeholder="Ask VAL about this transcript" onkeydown="if(event.key===\'Enter\')transcriptAsk()"><button class="val-ui-btn primary" onclick="transcriptAsk()">Ask</button></div></section></aside></div>';
}
