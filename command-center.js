(function(){
'use strict';
var transcriptState={items:[],counts:{total:0,needsReview:0,withOpenActions:0,failedProcessing:0},pagination:{offset:0,limit:100,total:0,hasMore:false},active:null,loaded:false,loading:false,error:'',lastLoadedAt:''};
...
function relationshipDossierHtml(dossier){
  ...
  return '<div class="relationship-profile-grid relationship-dossier">'
    ...
    +'<section class="exec-card"><h3>Evidence Notes</h3>'+list(brief.observerNotes,'No evidence note has earned space yet.')+'<div class="relationship-receipt-grid"><article><span>LinkedIn Signal</span><p>'+linkedInSignal()+'</p><button class="val-card-action-btn" onclick="relationshipDossierAction(\''+jsString(dossierId)+'\',\'review_linkedin_activity\')">Review LinkedIn activity</button></article><article><span>Source Evidence</span>'+observerList()+'<button class="val-card-action-btn" onclick="relationshipDossierAction(\''+jsString(dossierId)+'\',\'refresh_relationship_observers\')">Check for new evidence</button></article></div></section>'
...
function loadTranscripts(show,append){
  ...
  var offset=append?transcriptState.items.length:0;
  return fetcher((window.PROXY||'')+'/api/val/transcripts?days=3650&limit=100&offset='+offset).then(function(data){if(!data||data.ok===false||!Array.isArray(data.transcripts))throw new Error((data&&data.error)||'Transcript retrieval returned an invalid response.');transcriptState.items=append?transcriptState.items.concat(data.transcripts):data.transcripts;transcriptState.counts=data.counts||{total:transcriptState.items.length,needsReview:0,withOpenActions:0};transcriptState.pagination=data.pagination||{offset:offset,limit:100,total:transcriptState.items.length,hasMore:false};transcriptState.loaded=true;transcriptState.loading=false;transcriptState.error='';transcriptState.lastLoadedAt=new Date().toISOString();updateCommandCenterBadges();if(show||append)renderTranscriptList();return data;}).catch(function(e){transcriptState.loaded=true;transcriptState.loading=false;transcriptState.error=e.message||String(e);updateCommandCenterBadges();if(show)renderTranscriptError(transcriptState.error);throw e;});
}
window.loadMoreTranscripts=function(){if(transcriptState.loading||!transcriptState.pagination.hasMore)return;return loadTranscripts(false,true);};
...
function transcriptSidebar(activeId){
  ...
  var more=transcriptState.pagination&&transcriptState.pagination.hasMore?'<button class="val-ui-btn val-transcript-load-more" onclick="loadMoreTranscripts()">Load more</button>':'';
  return '<aside class="val-transcript-sidebar"><div class="val-transcript-sidebar-head"><span>Transcripts</span><strong>'+count+'</strong></div><div class="val-transcript-sidebar-list">'+(rows||'<div class="val-transcript-side-empty">No transcripts yet. Upload or connect a meeting source when ready.</div>')+'</div>'+more+'</aside>';
}
...
function renderTranscriptDetail(t){
  ...
  view.innerHTML=transcriptHeader(meta,true)+'<div class="val-transcript-shell detail">'+transcriptSidebar(t.id)+'<main class="val-transcript-main-detail"><div class="val-transcript-detail"><div class="val-detail-main"><section class="val-detail-card"><div class="val-detail-actions"><button class="val-ui-btn primary" onclick="transcriptAskFocus()">Co-Work on This Transcript</button><button class="val-ui-btn danger" onclick="deleteTranscript(\''+jsString(t.id)+'\')">Delete Transcript</button></div><h3>Summary</h3><p>'+safe(transcriptShortText(s.executiveSummary||s.clientSummary||'Summary pending.','Summary pending.',900))+'</p></section><section class="val-detail-card"><h3>Transcript</h3><p class="val-full-transcript">'+safe(t.transcriptText||t.rawTranscript||'No transcript text is available.')+'</p></section>'+debug+'</div><aside class="val-detail-side"><section class="val-detail-card"><h3>Co-Work on This Transcript</h3><div class="val-chat-log" id="valTranscriptChat"><div class="val-chat-msg">Ask about what happened, what was decided, what matters, or what VAL noticed in this transcript.</div></div><div class="val-chat-input"><input id="valTranscriptQuestion" placeholder="Ask VAL about this transcript" onkeydown="if(event.key===\'Enter\')transcriptAsk()"><button class="val-ui-btn primary" onclick="transcriptAsk()">Ask</button></div></section></aside></div></main></div>';
  var legacyCowork=document.getElementById('valTranscriptChat');
  if(legacyCowork&&legacyCowork.parentNode)legacyCowork.parentNode.innerHTML='<h3>Transcript Working Brief</h3><p>VAL will keep this meeting\'s exact Krisp receipt unchanged and prepare only reviewable internal work.</p><div id="valTranscriptWorkingBrief"><button class="val-ui-btn primary" onclick="transcriptAsk()">Open Transcript Working Brief</button></div>';
}
window.deleteTranscript=function(id){
  var phrase=prompt('Delete this transcript from VAL? This removes its transcript text and derived transcript records. Type delete transcript to continue.','');
  if(String(phrase||'').trim().toLowerCase()!=='delete transcript')return;
  return fetch((window.PROXY||'')+'/api/val/transcripts/'+encodeURIComponent(id),{method:'DELETE',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({confirmation:'delete transcript'})}).then(function(r){return r.json().then(function(d){if(!r.ok||d.ok===false)throw new Error(d.error||'Transcript deletion failed.');return d;});}).then(function(){transcriptState.items=transcriptState.items.filter(function(item){return String(item.id)!==String(id);});transcriptState.active=null;return loadTranscripts(true,false);}).catch(function(e){renderTranscriptError(e.message);});
};
...
function transcriptCoworkRequest(path,body){return fetch((window.PROXY||'')+path,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(body||{})}).then(function(r){return r.json().then(function(d){if(!r.ok||d.ok===false)throw new Error(d.error||'Transcript Working Brief failed.');return d;});});}
function renderTranscriptCoworkResult(data){var t=transcriptState.active,box=document.getElementById('valTranscriptWorkingBrief');if(!t||!box)return;t.coworkSessionId=data.session&&data.session.id||t.coworkSessionId||'';t.coworkWorkItemId=data.workItem&&data.workItem.id||t.coworkWorkItemId||'';var message=data.question&&data.question.question||data.message||'Transcript Working Brief ready.',status=data.workItem&&data.workItem.status||'';box.innerHTML='<p>'+safe(message)+'</p>'+(status==='needs_input'?'<button class="val-ui-btn primary" onclick="transcriptPrepareWorkingBrief()">Prepare exact meeting overview</button>':status==='needs_review'?'<button class="val-ui-btn primary" onclick="transcriptApplyWorkingBrief()">Create internal email draft</button>':status==='applied'?'<p>Internal draft is ready in Leverage. Nothing was sent.</p>':'');}
window.transcriptAsk=function(){var t=transcriptState.active;if(!t)return;if(!t.coworkSessionId)return transcriptCoworkRequest('/api/val/cowork/entries/open',{entrypointId:'transcript.working_brief',scope:{entityType:'transcript',entityId:t.id,sectionId:'working_brief'}}).then(renderTranscriptCoworkResult).catch(function(e){var box=document.getElementById('valTranscriptWorkingBrief');if(box)box.innerHTML='<p>'+safe(e.message)+'</p>';});return transcriptCoworkRequest('/api/val/cowork/sessions/'+encodeURIComponent(t.coworkSessionId)+'/respond',{answer:'prepare'}).then(renderTranscriptCoworkResult).catch(function(e){var box=document.getElementById('valTranscriptWorkingBrief');if(box)box.innerHTML='<p>'+safe(e.message)+'</p>';});};
window.transcriptPrepareWorkingBrief=function(){return window.transcriptAsk();};
window.transcriptApplyWorkingBrief=function(){var t=transcriptState.active;if(!t||!t.coworkWorkItemId)return;return transcriptCoworkRequest('/api/val/cowork/work-items/'+encodeURIComponent(t.coworkWorkItemId)+'/apply',{}).then(renderTranscriptCoworkResult).catch(function(e){var box=document.getElementById('valTranscriptWorkingBrief');if(box)box.innerHTML='<p>'+safe(e.message)+'</p>';});};
...
window.transcriptAskFromList=function(id){return openTranscriptDetail(id).then(function(){transcriptAsk();}).catch(function(){});};
...
window.transcriptAskFocus=function(){var box=document.getElementById('valTranscriptWorkingBrief');if(box){box.scrollIntoView({behavior:'smooth',block:'center'});transcriptAsk();}};
...
var originalSend=window.sendMessage;window.sendMessage=function(){return originalSend&&originalSend.apply(window,arguments);};
