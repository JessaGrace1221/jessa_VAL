...
const executive=fs.readFileSync(path.join(root,'val-executive.html'),'utf8');
...
test('voice playback uses server-side Deepgram TTS proxy instead of browser-side token calls',()=>{
  ...
  assert.match(executive,/\/api\/val\/tts/);
  assert.doesNotMatch(executive,/fetch\('https:\/\/api\.deepgram\.com\/v1\/speak/);
});
...
test('voice failures are visible and cadence is tightened',()=>{
