...
| Co-Work companion | `.cowork-notebook` | `cowork_packet` with `active_surface=home` | Co-Work prompt suite and `VAL_COWORK_WITH_VAL_V1_BUILD_SPEC.md` | Current Home state only unless source is selected | Think with VAL, Draft with VAL, Voice Co-Work | Do not send, save memory, or change CRM/calendar/tasks | Full private Co-Work workspace with conversation history |
...
| Project Managers drawer/card | `.project-drawer-link[disabled]` | Dormant `project_packet` index state | Project Managers rebuild hold | Preserved project source, linked people/docs/reviews | No public action while the rebuild is in progress | Do not open the drawer or create/update project records | Disabled `Project Managers Coming Soon` control |
...
| Project Co-Work | `data-project-cowork-scope="project_overview"` | `project_packet` plus `project_scoped_cowork_packet` | Project-scoped Co-Work rule | Current project packet and source receipts | Think, draft, compare, plan, summarize, ask questions, prepare review-only artifacts | Do not create external actions or borrow unrelated project context | Scoped Co-Work workspace |
| Project Manager action Co-Work | `data-project-cowork-field` | `project_packet` plus `project_scoped_cowork_packet` | Project Manager action Co-Work rule | Source receipts attached to the one selected project action/field | Add context, ask, refine, prepare review-only artifact, update selected local card text where applicable | Do not open unrelated documents, unrelated people, unrelated source memory, or broad project mutation | Item-scoped Co-Work workspace and action receipt |
...
| Forward | `data-correspondence-action="forward"` | `email_packet` | Forward draft rule | Selected email plus typed recipient/rule | Create private forward draft | Do not forward automatically | Internal draft receipt/status |
| Resolved | `data-correspondence-action="resolve_thread"` | `email_packet` | Resolution evidence rule | Selected message/thread id | Remove from active queue and keep evidence | Do not delete/archive provider mail | Resolution receipt |
| Executive contact | `data-correspondence-action="safe_contact"` / `save_safe_contact` | `email_packet` plus safe-list input | Executive contact admission rule | Selected sender or typed email | Save local Executive Inbox allow rule | Do not create unrelated CRM relationship | Safe contact receipt |
| Not executive contact | `data-correspondence-action="not_executive_contact"` | `email_packet` | Executive Inbox suppression rule | Selected sender/thread | Suppress future matching sender/domain according to rule scope | Do not delete source evidence | Suppression receipt |
| Attach relationship | `data-correspondence-action="link_relationship"` / `create_relationship` | `email_packet` plus `relationship_packet` | Relationship evidence-link rule | Selected email and chosen/created relationship | Save local evidence link or create reviewed relationship candidate | Do not infer broad relationship memory from weak evidence | Evidence link receipt |
| Attach project | `data-correspondence-action="link_project"` / `create_project` | `email_packet` plus `project_packet` | Project evidence-link rule | Selected email and chosen/created project | Save local evidence link or create reviewed project candidate | Do not invent project state | Evidence link receipt |
| Search inbox | `data-correspondence-action="search_inbox"` | `email_packet` search context | Inbox command search rule | Gmail/Outlook search result evidence | Add found results to review queue | Do not admit unrelated search results as executive by default | Search result/status |
| Create/view/suggest rules | `data-correspondence-action="create_rule"` / `show_rules` / `suggest_rules` / `save_forward_rule` | `email_packet` plus rule input | Rule learning review rule | Selected email and typed instruction | Save or suggest local rule after review | Do not auto-apply unclear rules externally | Rule receipt/status |
| Writing Rules | `data-correspondence-action="show_writing_rules"` / `save_draft_rules` | `drafts.current` plus user writing preferences | Draft voice rule map | Saved writing tone/signoff/do-not-say rules | Save local writing rule map and apply to new Executive Inbox drafts | Do not rewrite old drafts silently | Writing rule receipt/status |
