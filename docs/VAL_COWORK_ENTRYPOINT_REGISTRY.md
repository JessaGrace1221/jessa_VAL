...
| `project.people` | `People involved` | Connect the correct relationships and roles, including one project owner. | Each added person has a role; one owner is explicit. | `project_relationships_packet`, `project_owner_packet`; people card. |
...
| `project.milestones` | `Milestones` | Define evidence-based checkpoints. | Each milestone proves movement and names its completion signal. | `project_sop_packet.standard_milestones`; milestones card. |
...
|---|---|---|---|
| Project Managers | First three canonical slices complete | `project.identity`, `project.workstreams`, and `project.next_move` now have server-owned Working Briefs, field-targeted questions, structured prepared work, explicit internal Apply, action receipts, and return to the selected project. `Start onboarding chat`, `Project interview`, and `What this is` now use the project-foundation entry rather than generic chat. | Every other Project Managers entry still uses the older scoped-chat/write-back path and must be replaced through this registry before it is treated as canonical. |
