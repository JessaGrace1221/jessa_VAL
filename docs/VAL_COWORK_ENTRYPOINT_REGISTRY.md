...
| `project.identity` | `What this is` | Define the project, beneficiary/audience, and desired outcome. | Canonical name, purpose, and desired outcome are known. | `project_identity_packet`; `What this is`. |
...
| `project.onboarding` | `Start onboarding chat` / `Project interview` | Complete the project manager's foundational picture. | Name, outcome, owner, initial workstreams, milestones, nurture pattern, and prepared-work need are complete or explicitly deferred. | `project_interview_packet` plus the named Project Manager page sections. |
