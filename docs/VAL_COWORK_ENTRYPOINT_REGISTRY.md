...
| `project.identity` | `What this is` | Define the project, beneficiary/audience, desired outcome, and one project owner. | Canonical name, purpose, desired outcome, and owner are explicit. | `project_identity_packet` and `project_owner_packet`; `What this is`, `People involved`, and Working narrative. |
| `project.onboarding` | `Start onboarding chat`, `Continue onboarding chat`, and `Project Interview` | Continue the selected project through its protected onboarding sequence, one mapped page section at a time. | The exact current onboarding answer is mapped to its named page boxes, visible for review, and explicitly applied. | `project_onboarding_packet` plus only the named Project Manager fields; next reopen resumes at the next unanswered stage. |
...
`project.onboarding` is the one route behind `Start onboarding chat`, `Continue onboarding chat`, and `Project Interview`. It preserves these exact questions and their existing mapping: `What should this project be called, and what outcome should it create?` feeds Identity, What this is, and Working narrative; `Who owns this project, what is the next move, and what should VAL monitor next?` feeds People involved, Next move, and Monitoring after launch; `What are the main workstreams VAL should track for this project?` feeds Workstreams; `What milestones prove this project is moving?` feeds Milestones and Current phase; `How should VAL help protect and grow the relationships connected to this project?` feeds Relationship nurture; and `What should VAL prepare, organize, or ask about next for this project?` feeds Prepared work and What VAL needs next.

Each opening reads the selected project's durable onboarding status and resumes at only the next unanswered question. Each response becomes a reviewable internal stage packet that shows its exact answer, target packet field, and visible Project Manager boxes before `Apply`. Applying stores only that answer and its mapped internal fields for the selected project. It never infers missing project detail, copies another project, creates a task, sends a message, updates CRM, schedules anything, or alters source evidence.
...
| `transcript.action_item` | One selected transcript and one exact Krisp Action Item by source index. | Create one internal Commitment without rewriting, assigning, or broadening that source line. | The selected exact Action Item is visible for review and has an explicit internal Apply route. | One internal Commitment with the verbatim Action Item as title and source quote; receipt only. |
...
Transcript Working Brief opens the exact attendee overview already ready for review when its source receipt is present; it does not ask a redundant confirmation question. The Action Item route carries both `transcript_id` and the source Action Item index. Neither route may substitute a different meeting, alter Krisp text, assign a person, send a message, schedule time, or write to an external system.
...
| Entry id | Scope | Objective | Completion condition | Writes / result |
|---|---|---|---|---|
| `relationship.overview` | One selected durable relationship profile, current source receipts, and current internal stewardship state. | Prepare one source-aware next stewardship move. | One executive-directed next move, its source receipt, and a review-gated internal Apply are visible. | `relationship_stewardship_packet.next_move` and an internal action receipt. |
...
`relationship.overview` receives the selected relationship’s durable identifier at click time and rejects a missing relationship rather than borrowing a nearby person or a generic dossier. It asks exactly one question: what small relationship outcome VAL should prepare next. The answer creates one review-gated internal stewardship focus, grounded in the selected profile’s readable receipts. Applying writes only that selected relationship’s internal packet and receipt; it does not draft or send outreach, create a task, update CRM, make an introduction, schedule time, or alter an external system.

`relationship.section` is the contract behind the four clickable Stewardship cards: `needs`, `offers`, `relationship`, and `evidence`. It receives the exact relationship id and exactly one card id at click time. It asks only the question needed for that card, presents the user's exact answer as a reviewable internal update, and applies only after an explicit Apply. The result is stored as user-confirmed relationship context beside immutable source receipts, then flows into the durable person packet and Meeting Prep. It does not overwrite source material, enrich from an external provider, update any other person or field, create a task, send outreach, or alter an external system.
...
| Stewardship | Relationship overview and four field contracts complete | `relationship.overview` prepares one review-gated internal next stewardship move. `relationship.section` now opens one selected person's Needs, Offers, Relationship, or Evidence card, asks a field-targeted question, presents an exact internal update for review, and applies only that named card into user-confirmed relationship context, the durable person packet, and Meeting Prep. | New relationship sections require their own registered field contract before a Co-Work control is exposed. |
