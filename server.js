...
function guideHtml(markdown){
  ...
  const go = action => `/dashboard?guideAction=${encodeURIComponent(action)}`;
...
<section class="hero"><div><div class="eyebrow">Velocity-Activated Leverage</div><h1>VAL</h1><p>Your executive operating layer. Never lose track of important people, promises, or opportunities again.</p><div class="actions"><a class="btn" href="${go('today')}">Open Today</a><a class="btn secondary" href="${go('radar')}">Run Radar</a></div></div></section>
...
<a class="card" href="${go('meetings')}"><span class="icon">${icon.calendar}</span><h3>Prepare For Today</h3><p>Know who matters before your next conversation.</p><div class="status" id="meetingStatus">Loading meetings</div></a>
<a class="card" href="${go('radar')}"><span class="icon">${icon.radar}</span><h3>Relationship Radar</h3><p>See who needs follow-up before momentum dies.</p><div class="status" id="radarStatus">Checking signals</div></a>
<a class="card" href="${go('approvals')}"><span class="icon">${icon.stack}</span><h3>Approval Queue</h3><p>Review drafts, promises, and pending actions.</p><div class="status" id="queueStatus">Loading drafts</div></a>
</div></section>
<section><div class="section-head"><div><h2>Your First 3 Minutes</h2><p>A short path that helps VAL understand you and start creating momentum.</p></div></div><div class="journey"><div class="step"><span>Step 1</span><h3>Personalize VAL</h3><p>Tell VAL who you are, how you work, and what relationships drive your business.</p><a class="btn secondary" href="${go('onboarding')}">Personalize VAL</a></div><div class="step"><span>Step 2</span><h3>Review Today</h3><p>See meetings, priorities, and what needs your attention before the day gets noisy.</p><a class="btn secondary" href="${go('today')}">Open Today View</a></div><div class="step"><span>Step 3</span><h3>Run Radar</h3><p>Find the people and promises most likely to create value or lose trust if ignored.</p><a class="btn secondary" href="${go('radar')}">Run Relationship Radar</a></div></div></section>
<section><div class="section-head"><div><h2>What Do You Want To Do?</h2><p>Choose by outcome, not by feature name.</p></div></div><div class="modes"><div class="mode"><h3>Stay Ahead</h3><a href="${go('meetings')}">Meeting Prep</a><a href="${go('rhythm')}">Daily Rhythm</a><a href="${go('calendar')}">Calendar Intelligence</a></div><div class="mode"><h3>Protect Relationships</h3><a href="${go('radar')}">Relationship Radar</a><a href="${go('followups')}">Follow-Ups</a><a href="${go('contacts')}">Contact Command Center</a></div><div class="mode"><h3>Clear Mental Load</h3><a href="${go('approvals')}">Approval Queue</a><a href="${go('drafts')}">Drafts</a><a href="${go('tasks')}">Tasks By Relationship</a></div><div class="mode"><h3>Think Better</h3><a href="${go('voice')}">Voice Mode</a><a href="${go('reflection')}">Executive Reflection</a><a href="${go('savedtime')}">Use Saved Time</a></div></div></section>
...
async function recentMemoryContext(query){
  ...
  const recentDocumentPinned = (items)=>{
    ...
    return items
      .filter(m=>/knowledge_document|processed_transcript|transcript/i.test(m.kind||''))
      .sort((a,b)=>{
  ...
  if(pgPool){
    ...
    return format(uniqueByContent(coreProfilePinned(r.rows).concat(recentDocumentPinned(r.rows),ranked)).slice(0,documentMode?22:(identityMode?24:18)));
  }
  ...
  const ranked = storeItems.map(m=>({...m,_score:scoreMemory(m,terms)}))
    ...
    .sort((a,b)=>(b._score-a._score)||((b.importance||1)-(a.importance||1)))
    .slice(0,identityMode?22:14);
  return format(uniqueByContent(coreProfilePinned(storeItems).concat(recentDocumentPinned(storeItems),ranked)).slice(0,documentMode?22:(identityMode?24:18)));
}
...
app.post('/api/val/chat',async(req,res)=>{
  try{
    ...
    const memoryQuery = messages.slice(-10).map(m=>m.content||'').join('\n').slice(-6000);
